...
fn main()->std::io::Result<()>{
    ...
    let reads:Vec<_> = fastq::Reader::from_file(&Path::new(&args[1])).unwrap()
        .records()
        .filter_map(|e|e.ok())
        .collect();
    let read_fin = std::time::Instant::now();
    let result:Vec<_> =  reads.into_par_iter()
        .map(trim_self_chimera)
        .collect();
    let end = std::time::Instant::now();
    let mut wtr = fastq::Writer::new(std::io::stdout());
    result.into_iter().for_each(|read| wtr.write_record(&read).unwrap());
...
fn trim_self_chimera(read:fastq::Record)->fastq::Record{
    ...
    if let Some((start,end)) = determine(&result){
        ...
        fastq::Record::with_attrs(read.id(),
                                  read.desc(),
                                  seq,qual)
    }else{
        read
