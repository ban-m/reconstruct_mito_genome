...
fn main()->std::io::Result<()>{
    ...
    let fasta:HashMap<_,_> = fasta::Reader::from_file(
        &Path::new(&args[1]))?
    .records()
        .filter_map(|e|e.ok())
    ...
    for (contig,position) in BufReader::new(
        File::open(&Path::new(&args[2]))?)
        .lines()
        .filter_map(|e|e.ok())
        .filter_map(|e|{
            ...
            let bucket = splits.entry(contig)
                .or_insert(vec![0,length]);
    ...
    for (contig, positions) in splits.iter_mut(){
        ...
        for window in positions.windows(2){
            ...
            wtr.write(&record,
                      None,
                      &fasta[contig].seq()[window[0]..window[1]])?;
            id += 1;
        }
    }
    Ok(())
