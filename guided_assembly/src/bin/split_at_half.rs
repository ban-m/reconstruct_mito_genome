...
fn main() -> std::io::Result<()>{
    ...
    for record in fasta::Reader::from_file(&Path::new(path))?.records()
        .filter_map(|e|e.ok()).map(|e|split_at_half(e, overlap_len)){
...
fn split_at_half(record:fasta::Record,overlap_len:usize)-> fasta::Record{
    ...
    fasta::Record::with_attrs(record.id(),record.desc(),&[latter, &former[overlap_len..]].concat())
}
