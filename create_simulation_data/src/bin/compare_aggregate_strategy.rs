...
fn main() {
    ...
    let result: Vec<_> = sample_num
        .into_par_iter()
        .map(|(seed, coverage)| {
            let (naive, w, wl, dist) = benchmark(seed, p, coverage, test_num, chain_len, k, len);
            (naive, w, wl, dist, coverage)
        })
        .collect();
    println!("Niave\tWeightedSum\tWeightedLikelihood\tDist\tCoverage\tLength");
    for (x, y, z, dist, coverage) in result {
        println!(
            "{}\t{}\t{}\t{}\t{}\t{}",
            x,
            y,
...
fn benchmark(
    ...
    let (x, y, z) = tests
        ...
        .map(|&(ans, ref test)| {
            ...
            let (x, y, z) = merge_predict(&l1, &l2);
            (ans == x, ans == y, ans == z)
        })
        .fold((0., 0., 0.), |(x, y, z), (a, b, c)| {
            (add(x, a), add(y, b), add(z, c))
        });
...
fn merge_predict(l1: &[f64], l2: &[f64]) -> (u8, u8, u8) {
