extern crate bio_utils;
extern crate poa_hmm;
extern crate rand;
extern crate rand_xoshiro;
use poa_hmm::*;
use rand::Rng;
use rand::SeedableRng;
use rand_xoshiro::Xoshiro256StarStar;
fn main() -> std::io::Result<()> {
    let args: Vec<_> = std::env::args().collect();
    let seed = 2312789;
    let mut rng: Xoshiro256StarStar = SeedableRng::seed_from_u64(seed);
    let reference_len: usize = args[1].parse().unwrap();
    let template1 = gen_sample::generate_seq(&mut rng, reference_len);
    let (sub, del, ins) = {
        let (mut sub, mut del, mut ins) = (1, 1, 1);
        let v_sites = reference_len / 2_000;
        for _ in 0..v_sites {
            match rng.gen_range(0, 3) {
                0 => sub += 1,
                1 => del += 1,
                2 => ins += 1,
                _ => {}
            }
        }
        (sub, del, ins)
    };
    eprintln!("{}\t{}\t{}", sub, del, ins);
    let template2 = gen_sample::introduce_errors(&template1, &mut rng, sub, del, ins);
    let stdout = std::io::stdout();
    let mut wtr = bio_utils::fasta::Writer::new(stdout.lock());
    let desc1 = Some("depth=0.5 circular=true".to_string());
    let desc2 = Some("depth=0.5 circular=true".to_string());
    let record1 =
        bio_utils::fasta::Record::with_data(&format!("sample1:{}", seed), &desc1, &template1);
    let record2 =
        bio_utils::fasta::Record::with_data(&format!("sample2:{}", seed), &desc2, &template2);
    wtr.write_record(&record1)?;
    wtr.write_record(&record2)
}
