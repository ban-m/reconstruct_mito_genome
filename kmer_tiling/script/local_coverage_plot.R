...
### ------ Load data -----
## args <- c("./result/10_exampler.dat",
##           "./result/sequel_minimap2_assignment.tsv",10)
...
plot_each_coverage <- function(ls){
    ...
    cowplot::ggsave(filename = "./pdf/cov_" %s+% k %s+% "_" %s+% name %s+% ".pdf")
    cowplot::ggsave(filename = "./png/cov_" %s+% k %s+% "_" %s+% name %s+% ".png")
}
