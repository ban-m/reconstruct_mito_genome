...
cargo run --release --bin annotate_dotplot -- ${CONTIGS} ${SELF_LASTTAB} \
      > ./data/repeats.json
cargo run --release --bin encode -- ${LASTTAB} ${CONTIGS} ${READ} ./data/repeats.json \
      ./data/contigs.json ./data/reads.json \
      2> ./logfiles/test.log  > ./logfiles/test.out
cargo run --release --bin convert_to_d3_data -- ./data/contigs.json ./data/reads.json \
      > ./data/d3.json
