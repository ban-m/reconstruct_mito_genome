...
do
    ...
    cargo run --release --bin create_mock_genomes -- ${LEN} 0.01 2 13223 \
          ${OUTPUT}/${coverage}_01/${coverage}_01
    badread simulate \
    ...
    cat ${OUTPUT}/${coverage}_01/${coverage}_01_reads.fq |\
        paste - - - - | cut -f 1,2 |\
        sed -e 's/@/>/g' | tr '\t' '\n' \
                              > ${OUTPUT}/${coverage}_01/${coverage}_01_reads.fa
    ## Mut 005
    mkdir -p ${OUTPUT}/${coverage}_005
    cargo run --release --bin create_mock_genomes -- ${LEN} 0.005 2 13223 \
          ${OUTPUT}/${coverage}_005/${coverage}_005
    badread simulate \
    ...
    cat ${OUTPUT}/${coverage}_005/${coverage}_005_reads.fq |\
        paste - - - - | cut -f 1,2 |\
        sed -e 's/@/>/g' | tr '\t' '\n' \
                              > ${OUTPUT}/${coverage}_005/${coverage}_005_reads.fa
    ## Mut 001
